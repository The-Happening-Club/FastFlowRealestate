    ...
    <main className="flex flex-col justify-center items-center h-svh w-svw">
      <div className="flex justify-end items-center bg-slate-100 w-[1920px] h-[1080px] rounded-3xl overflow-hidden">
        <div className=" flex flex-col h-full bg-slate-200 w-96 overflow-hidden">
          ...
          <div className="relative flex h-20 bg-slate-300 m-2 rounded-xl items-center gap-2 p-2">
            <div className="flex w-fit max-w-20 relative">
              <div className="relative w-10 h-10 bg-slate-400 rounded-full border-2 aspect-square border-primary"></div>
              <div className="relative -left-1/4 w-10 h-10 bg-slate-400 rounded-full border-2 aspect-square border-primary"></div>
              <div className="relative -left-1/2 w-10 h-10 bg-slate-400 rounded-full border-2 aspect-square border-primary"></div>
            </div>
            <span className="text-slate-700 font-bold">WEG 236</span>
            <span className="text-slate-700">Rohrbruch</span>
        ...
        <div className="flex flex-col w-full h-full justify-end items-end transition-all duration-500 ease-in-out">
          <div className="flex flex-col w-full h-full justify-end items-end transition-all duration-500 ease-in-out">
            <div className="flex max-w-56 h-fit text-white p-4 m-4 rounded-3xl even:text-black odd:text-white even:bg-red-500 odd:bg-primary">
            ...
            {messeges.map((message, i) => (
              <div
                key={i}
                className="flex max-w-56 h-fit text-white p-4 m-4 rounded-3xl even:text-black odd:text-white even:bg-neutral-100 odd:bg-primary animate-fadeIn"
              >
                {message}
              </div>
            ))}
          </div>
          <div className="flex w-full bg-white p-6 z-10 gap-4">
            <input
              type="text"
              className="w-full px-[12px] py-[12px] rounded-[14px] text-[16px] border bg-white border-slate-200 focus:outline-primary"
              placeholder="type your message"
              onChange={(e) => setMessege(e.target.value)}
              value={messege}
            />
            <button
              className="h-14 w-14 rounded-full text-white bg-primary"
